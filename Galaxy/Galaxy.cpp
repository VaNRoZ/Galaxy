...
struct Body
{
	...
	{
		float dist = sqrt((pos - othet.pos)*(pos - other.pos));
...
int main()
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		float theta = random(0.0f, 2 * M_PI);										//angle the particle makes with the center

		float r = random(1.0f, maxRadius);
		r = r * r / maxRadius;														//change distribution of particles (optional)
		r += 0.2f*Centre1.r;
		Bodies1[i].pos = { r*cos(theta), r*sin(theta) };							//polar to cartezian
		Bodies1[i].pos += Centre1.pos;												//move the particle relative to the centre

		float c = sqrt(G*Centre1.m / r);											//calculare velocity Based on radius
		Bodies1[i].vel = { v*sin(theta, r*sin(theta)) };							//polar to cartezian coordinates, rotated by 90 degrees
		float offset = 0.6f;
